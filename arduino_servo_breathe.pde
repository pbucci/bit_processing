...
void setup() {
  ...
  arduino.pinMode(pin_round, Arduino.SERVO);
  arduino.pinMode(pin_spine, Arduino.SERVO);
}
...
void draw() {
  ...
  if(isShowing()) {
    windw = getLocationOnScreen();
    if (mouse.x < windw.x + width && 
        mouse.y < windw.y + height &&
        mouse.x > windw.x &&
        mouse.y > windw.y) {
      println("In window");
      ...
      // rotation servos).
      int deg = int(map(mouseX,0,width,0,180));
      println("mouse " + deg);
      arduino.servoWrite(pin_round, deg);
      arduino.servoWrite(pin_spine, deg);
    }
    else {
      println("Out of window");
      arduino.servoWrite(pin_round,int(degrees));
      arduino.servoWrite(pin_spine,int(degrees));
      degrees = degrees + adder;
      println(degrees);
      if (degrees > maxdegrees - add || degrees < mindegrees + add) {
        println("multiplied by 1");
        adder = adder * (-1);
      }
